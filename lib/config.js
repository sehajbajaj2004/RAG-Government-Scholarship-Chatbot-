...
// Coverage pass (lib/retrieve.js): best chunk from each of up to this many distinct
// documents. Sized to the corpus — 10 PDFs today — so every scheme can appear.
export const COVERAGE_GROUPS = 10;

// Hard ceiling on chunks sent to Gemini, across both retrieval passes. Bounds the
// prompt at roughly 16 × ~500 tokens ≈ 8k tokens of context, which keeps requests
// inside the free-tier per-minute token budget.
export const MAX_CONTEXT_CHUNKS = 16;

// Spec §2 names Gemini 2.5 Flash. Deviated to Flash-Lite because the free tier allows
// only 20 requests/day/model for gemini-2.5-flash, which is not enough to test with.
// The quota is per project *per model*, so this is a separate bucket rather than a
// share of the same one. Switch back by restoring 'gemini-2.5-flash'.
